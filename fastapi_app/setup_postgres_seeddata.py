...
async def seed_data(engine: AsyncEngine) -> None:
    ...
    async with engine.begin() as conn:
        ...
        if not result.scalar():
            logger.error(
                f" {table_name} table does not exist. Please run the database setup script first."
            )
            return
        ...
        with open(os.path.join(current_dir, "seed_data.json")) as f:
            ...
            for seed_data_object in seed_data_objects:
                db_item = await session.execute(
                    select(Item).filter(Item.id == seed_data_object["id"])
                )
                if db_item.scalars().first():
                    continue
                attrs = {key: value for key, value in seed_data_object.items()}
                attrs["embedding_ada002"] = np.array(
                    seed_data_object["embedding_ada002"]
                )
