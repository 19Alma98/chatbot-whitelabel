...
async def create_postgres_engine(
    *,
    ...
    def get_password_from_azure_credential() -> str:
        token = azure_credential.get_token(  # type: ignore
    ...
    if host.endswith(".database.azure.com"):
        ...
        if azure_credential is None:
            raise ValueError(
                "Azure credential must be provided for Azure Database for PostgreSQL"
            )
    ...
    def register_custom_types(dbapi_connection: AdaptedConnection, *args: Any) -> None:
        logger.info("Registering pgvector extension...")
        try:
            dbapi_connection.run_async(register_vector)
        except ValueError:
            logger.warning(
                "Could not register pgvector data type yet as vector extension has not been CREATEd"
            )
...
async def create_postgres_engine_from_env(
    ...
    if azure_credential is None and os.environ["POSTGRES_HOST"].endswith(
        ".database.azure.com"
    ):
...
async def create_postgres_engine_from_args(
    args: Any, azure_credential: Any | None = None
) -> AsyncEngine:
