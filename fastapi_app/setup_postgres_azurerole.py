...
async def assign_role_for_webapp(engine: AsyncEngine, app_identity_name: str) -> None:
    async with engine.begin() as conn:
        ...
        if identities.rowcount > 0:
            logger.info(
                f"Found an existing PostgreSQL role for identity {app_identity_name}"
            )
        else:
            logger.info(f"Creating a PostgreSQL role for identity {app_identity_name}")
            await conn.execute(
                text(
                    f"SELECT * FROM pgaadauth_create_principal('{app_identity_name}', false, false)"
                )
            )
        ...
        await conn.execute(
            text(f'GRANT USAGE ON SCHEMA public TO "{app_identity_name}"')
        )
        await conn.execute(
            text(f'GRANT CREATE ON SCHEMA public TO "{app_identity_name}"')
        )
        await conn.execute(
            text(
                f'GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO "{app_identity_name}"'
            )
        )
        await conn.execute(
            text(
...
async def main() -> None:
    ...
    parser.add_argument(
        "--database", type=str, help="Postgres database", default="postgres"
    )
    ...
    parser.add_argument(
        "--app-identity-name", type=str, help="Azure App Service identity name"
    )

    args = parser.parse_args()
    if not args.host.endswith(".database.azure.com"):
        logger.info(
            "This script is intended to be used with Azure Database for PostgreSQL, not local PostgreSQL."
        )
        return
